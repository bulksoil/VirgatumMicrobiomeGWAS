...
type <- sommer_mods %>% 
  ...
  filter(type > 1) %>% 
  mutate(type = map(mod, ~extract_type(.))) %>% 
  unnest(type) %>% dplyr::select(ASVID, type)

tre <- read_rds("~/Box/GWAS_Paper/combined_tree.rds.gz")$tree
type2 <- gxe_p %>% 
  mutate(p = ifelse(type == "GxE", -log10(as.numeric(gxeP)), ifelse(type == "G", -log10(as.numeric(gP)), 0))) %>% 
  filter(ASVID%in%tre$tip.label)
pruned.tree<-drop.tip(tre,tre$tip.label[-match(type2$ASVID, tre$tip.label)])
tre_labels <- data.frame(tip.label = as.character(pruned.tree$tip.label)) %>% 
  mutate(ASVID = tip.label) %>% 
  inner_join(tax) %>% 
  mutate(Phylum2 = ifelse(Phylum == "Proteobacteria", as.character(Class), as.character(Phylum))) %>% 
  mutate(Genus = ifelse(grepl("Rhizobium", Genus), "Rhizobium", Genus),
         Genus = ifelse(grepl("Burkholderia", Genus), "Burkholderia", Genus)) %>% 
  mutate(unit = ifelse(is.na(Genus), as.character(Family), Genus),
         unit = ifelse(is.na(unit), as.character(Order), unit),
         unit = ifelse(is.na(unit), as.character(Class), unit)) %>% 
  mutate(Phylum2 = fct_lump(Phylum2, n = 7)) %>% 
  inner_join(type2)
tree_plot <- ggtree::ggtree(pruned.tree, color = "grey70", layout = "circular") +xlim(-0.25, NA)
tax_tree <- tree_plot %<+% tre_labels  + 
  geom_tiplab(align = T, hjust = 0, aes(label = unit, color = Phylum2), size = 3, offset = 0.05) +
  geom_tippoint(hjust = 0, aes(fill = type, size=p, alpha = type), offset = 0.05, shape = 21, stroke = 0) +
  scale_x_continuous(expand=c(0, 0.5)) + 
  scale_alpha_manual(values = c(1,1,0)) +
  scale_fill_manual(values = rev(c("white", "#9EBCDA", "#8856A7"))) +
  scale_color_manual(values = c("#9c003d", "#d43a4a", "#66c2a1", "#f3683f", "#fdae61", "#2e85bd","#5c4aa1", "grey50", "#abdca1", "white", "black")) +
